...
public class SwipeLayout extends FrameLayout implements View.OnTouchListener, View.OnClickListener {
    ...
    public void changeItemText(boolean left, String text, int index){
        if(left){
            ArrayList<View> allViewsWithinMyTopView = getAllChildren(leftViews[index]);
            ...
            ArrayList<View> allViewsWithinMyTopView = getAllChildren(rightViews[index]);
            for (View child : allViewsWithinMyTopView) {
                if (child instanceof TextView) {
                    TextView childTextView = (TextView) child;
                    childTextView.setText(text);
    ...
    private void addSwipeItems(int[] icons, int[] iconColors, int[] backgroundColors, String[] texts, int[] textColors,
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            if(i > 0 && separatorWidth > 0){
                ...
                separatorView.setLayoutParams(new LinearLayout.LayoutParams(separatorWidth, ViewGroup.LayoutParams.MATCH_PARENT));
                separatorView.setBackgroundColor(separatorColor);

                if (i == icons.length - (!left ? 1 : icons.length)) {
                    layout.addView(separatorView);
                } else {
                    layoutWithout.addView(separatorView);
                }
            }
    ...
    private void setUpAttrs(AttributeSet attrs) {
        ...
        if (array != null) {
            ...
            separatorWidth = array.getDimensionPixelSize(R.styleable.SwipeLayout_separatorWidth, 0);
            separatorColor = array.getColor(R.styleable.SwipeLayout_separatorColor, 0);
